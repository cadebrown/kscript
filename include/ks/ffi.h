/* ks/ffi.h - kscript 'ffi' module C-API
 *
 * The 'ffi' module is meant to be a cross-platform interface to other existing C-libraries on the system
 *   and loading symbols, calling them, and describing C-style structures.
 * 
 * 
...
typedef struct ksffi_dll_s {
    ...
    /* Opaque handle, which is returned by 'dlopen()' or 
     *   similar
     */
    void* handle;
...
#define KSFFI_TYPE_I(_name, _type) \
typedef struct ksffi_##_name##_s { \
    KSO_BASE \
    _type val; \
}* ksffi_##_name;

KSFFI_DO_I(KSFFI_TYPE_I)

/** Define floating point types **/
#define KSFFI_TYPE_F(_name, _type) \
typedef struct ksffi_##_name##_s { \
    KSO_BASE \
    _type val; \
}* ksffi_##_name;

KSFFI_DO_F(KSFFI_TYPE_F)
 ...
 * 'sizeofp' calculates the size of '*tp', which must be a pointer type
 * Returns -1 if 'tp' was not a C-style type, or did not fit expectations
 */
...
/* Types */
KS_API_DATA ks_type
#define KSFFI_DECL_I(_name, _type) ksffit_##_name,
KSFFI_DO_I(KSFFI_DECL_I)

#define KSFFI_DECL_F(_name, _type) ksffit_##_name,
KSFFI_DO_F(KSFFI_DECL_F)
