...
#define T ks_list
#define T_TYPE kst_list
#define T_NAME "list"
#define T_DOC "Mutable collection of other objects, arranged in a linear fashion and indexable"


/* C-API */

ks_list ks_list_new(ks_type tp, ks_uint len, kso* elems) {
    if (!tp) tp = T_TYPE;

    /* Create new elements array first, so we don't allocate the object too */
    kso* new_elems = ks_malloc(sizeof(*new_elems) * len);
    if (!new_elems) {
        KS_THROW_OOM("While allocating list");
        return NULL;
    }

    /* Allocate new object */
    ks_list self = KS_NEW(ks_list, tp);
    if (!self) {
        ks_free(new_elems);
        return NULL;
    }
