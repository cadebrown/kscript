...
static int kern(int N, nxar_t* inp, int len, void* _data) {
    ...
    if (dtype == nxd_float) {
        ...
        for (i = 0; i < len; i++, pR += sR, pX += sX) {
            ...
            *(TYPE*)pR = acosf(vX);
        #else
            KS_THROW(kst_Error, "Kernel '%s' is not supported for dtype %R, function(s) not found: acosf", K_NAME, dtype);
            return 1;
        #endif
        }
        ...
        #ifdef KS_HAVE_acos
            *(TYPE*)pR = acos(vX);
        #elif defined(KS_HAVE_acosf)
            *(TYPE*)pR = acosf(vX);
        #else
            KS_THROW(kst_Error, "Kernel '%s' is not supported for dtype %R, function(s) not found: acos, acosf", K_NAME, dtype);
            return 1;
        #endif
        }
        ...
        #ifdef KS_HAVE_acosl
            *(TYPE*)pR = acosl(vX);
        #elif defined(KS_HAVE_acos)
            *(TYPE*)pR = acos(vX);
        #elif defined(KS_HAVE_acosf)
            *(TYPE*)pR = acosf(vX);
        #else
            KS_THROW(kst_Error, "Kernel '%s' is not supported for dtype %R, function(s) not found: acosl, acos, acosf", K_NAME, dtype);
            return 1;
        #endif
        }
        ...
        for (i = 0; i < len; i++, pR += sR, pX += sX) {
            ...
            *(TYPE*)pR = acosf(vX);
        #else
            KS_THROW(kst_Error, "Kernel '%s' is not supported for dtype %R, function(s) not found: acosf128, acosl, acos, acosf", K_NAME, dtype);
            return 1;
        #endif
        }
...
bool nx_acos(nxar_t r, nxar_t x) {
