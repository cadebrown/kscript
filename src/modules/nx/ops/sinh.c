...
static int kern(int N, nxar_t* inp, int len, void* _data) {
    ...
    if (dtype == nxd_float) {
        ...
        for (i = 0; i < len; i++, pR += sR, pX += sX) {
            ...
            *(TYPE*)pR = sinhf(vX);
        #else
            KS_THROW(kst_Error, "Kernel '%s' is not supported for dtype %R, function(s) not found: sinhf", K_NAME, dtype);
            return 1;
        #endif
        }
        ...
        #ifdef KS_HAVE_sinh
            *(TYPE*)pR = sinh(vX);
        #elif defined(KS_HAVE_sinhf)
            *(TYPE*)pR = sinhf(vX);
        #else
            KS_THROW(kst_Error, "Kernel '%s' is not supported for dtype %R, function(s) not found: sinh, sinhf", K_NAME, dtype);
            return 1;
        #endif
        }
        ...
        #ifdef KS_HAVE_sinhl
            *(TYPE*)pR = sinhl(vX);
        #elif defined(KS_HAVE_sinh)
            *(TYPE*)pR = sinh(vX);
        #elif defined(KS_HAVE_sinhf)
            *(TYPE*)pR = sinhf(vX);
        #else
            KS_THROW(kst_Error, "Kernel '%s' is not supported for dtype %R, function(s) not found: sinhl, sinh, sinhf", K_NAME, dtype);
            return 1;
        #endif
        }
        ...
        for (i = 0; i < len; i++, pR += sR, pX += sX) {
            ...
            *(TYPE*)pR = sinhf(vX);
        #else
            KS_THROW(kst_Error, "Kernel '%s' is not supported for dtype %R, function(s) not found: sinhf128, sinhl, sinh, sinhf", K_NAME, dtype);
            return 1;
        #endif
        }
...
bool nx_sinh(nxar_t r, nxar_t x) {
