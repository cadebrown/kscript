...
static int kern(int N, nxar_t* inp, int len, void* _data) {
    ...
    if (dtype == nxd_float) {
        ...
        for (i = 0; i < len; i++, pR += sR, pX += sX) {
            ...
            *(TYPE*)pR = tanhf(vX);
        #else
            KS_THROW(kst_Error, "Kernel '%s' is not supported for dtype %R, function(s) not found: tanhf", K_NAME, dtype);
            return 1;
        #endif
        }
        ...
        #ifdef KS_HAVE_tanh
            *(TYPE*)pR = tanh(vX);
        #elif defined(KS_HAVE_tanhf)
            *(TYPE*)pR = tanhf(vX);
        #else
            KS_THROW(kst_Error, "Kernel '%s' is not supported for dtype %R, function(s) not found: tanh, tanhf", K_NAME, dtype);
            return 1;
        #endif
        }
        ...
        #ifdef KS_HAVE_tanhl
            *(TYPE*)pR = tanhl(vX);
        #elif defined(KS_HAVE_tanh)
            *(TYPE*)pR = tanh(vX);
        #elif defined(KS_HAVE_tanhf)
            *(TYPE*)pR = tanhf(vX);
        #else
            KS_THROW(kst_Error, "Kernel '%s' is not supported for dtype %R, function(s) not found: tanhl, tanh, tanhf", K_NAME, dtype);
            return 1;
        #endif
        }
        ...
        for (i = 0; i < len; i++, pR += sR, pX += sX) {
            ...
            *(TYPE*)pR = tanhf(vX);
        #else
            KS_THROW(kst_Error, "Kernel '%s' is not supported for dtype %R, function(s) not found: tanhf128, tanhl, tanh, tanhf", K_NAME, dtype);
            return 1;
        #endif
        }
...
bool nx_tanh(nxar_t r, nxar_t x) {
