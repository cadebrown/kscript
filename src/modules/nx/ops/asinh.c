...
static int kern(int N, nxar_t* inp, int len, void* _data) {
    ...
    if (dtype == nxd_float) {
        ...
        for (i = 0; i < len; i++, pR += sR, pX += sX) {
            TYPE vX = *(TYPE*)pX;
        #ifdef KS_HAVE_asinhf
            *(TYPE*)pR = asinhf(vX);
        #else
            KS_THROW(kst_Error, "Kernel '%s' is not supported for dtype %R, function(s) not found: asinhf", K_NAME, dtype);
            return 1;
        #endif
        }
        ...
        #ifdef KS_HAVE_asinh
            *(TYPE*)pR = asinh(vX);
        #elif defined(KS_HAVE_asinhf)
            *(TYPE*)pR = asinhf(vX);
        #else
            KS_THROW(kst_Error, "Kernel '%s' is not supported for dtype %R, function(s) not found: asinh, asinhf", K_NAME, dtype);
            return 1;
        #endif
        }
        ...
        for (i = 0; i < len; i++, pR += sR, pX += sX) {
            TYPE vX = *(TYPE*)pX;
        #ifdef KS_HAVE_asinhl
            *(TYPE*)pR = asinhl(vX);
        #elif defined(KS_HAVE_asinh)
            *(TYPE*)pR = asinh(vX);
        #elif defined(KS_HAVE_asinhf)
            *(TYPE*)pR = asinhf(vX);
        #else
            KS_THROW(kst_Error, "Kernel '%s' is not supported for dtype %R, function(s) not found: asinhl, asinh, asinhf", K_NAME, dtype);
            return 1;
        #endif
        }
        ...
        for (i = 0; i < len; i++, pR += sR, pX += sX) {
            TYPE vX = *(TYPE*)pX;
        #ifdef KS_HAVE_asinhf128
            *(TYPE*)pR = asinhf128(vX);
            ...
            *(TYPE*)pR = asinhf(vX);
        #else
            KS_THROW(kst_Error, "Kernel '%s' is not supported for dtype %R, function(s) not found: asinhf128, asinhl, asinh, asinhf", K_NAME, dtype);
            return 1;
        #endif
        }
...
bool nx_asinh(nxar_t r, nxar_t x) {
