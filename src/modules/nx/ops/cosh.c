...
static int kern(int N, nxar_t* inp, int len, void* _data) {
    ...
    if (dtype == nxd_float) {
        ...
        for (i = 0; i < len; i++, pR += sR, pX += sX) {
            ...
            *(TYPE*)pR = coshf(vX);
        #else
            KS_THROW(kst_Error, "Kernel '%s' is not supported for dtype %R, function(s) not found: coshf", K_NAME, dtype);
            return 1;
        #endif
        }
        ...
        #ifdef KS_HAVE_cosh
            *(TYPE*)pR = cosh(vX);
        #elif defined(KS_HAVE_coshf)
            *(TYPE*)pR = coshf(vX);
        #else
            KS_THROW(kst_Error, "Kernel '%s' is not supported for dtype %R, function(s) not found: cosh, coshf", K_NAME, dtype);
            return 1;
        #endif
        }
        ...
        #ifdef KS_HAVE_coshl
            *(TYPE*)pR = coshl(vX);
        #elif defined(KS_HAVE_cosh)
            *(TYPE*)pR = cosh(vX);
        #elif defined(KS_HAVE_coshf)
            *(TYPE*)pR = coshf(vX);
        #else
            KS_THROW(kst_Error, "Kernel '%s' is not supported for dtype %R, function(s) not found: coshl, cosh, coshf", K_NAME, dtype);
            return 1;
        #endif
        }
        ...
        for (i = 0; i < len; i++, pR += sR, pX += sX) {
            ...
            *(TYPE*)pR = coshf(vX);
        #else
            KS_THROW(kst_Error, "Kernel '%s' is not supported for dtype %R, function(s) not found: coshf128, coshl, cosh, coshf", K_NAME, dtype);
            return 1;
        #endif
        }
...
bool nx_cosh(nxar_t r, nxar_t x) {
