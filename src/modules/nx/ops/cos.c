...
static int kern(int N, nxar_t* inp, int len, void* _data) {
    ...
    if (dtype == nxd_float) {
        ...
        for (i = 0; i < len; i++, pR += sR, pX += sX) {
            ...
            *(TYPE*)pR = cosf(vX);
        #else
            KS_THROW(kst_Error, "Kernel '%s' is not supported for dtype %R, function(s) not found: cosf", K_NAME, dtype);
            return 1;
        #endif
        }
        ...
        #ifdef KS_HAVE_cos
            *(TYPE*)pR = cos(vX);
        #elif defined(KS_HAVE_cosf)
            *(TYPE*)pR = cosf(vX);
        #else
            KS_THROW(kst_Error, "Kernel '%s' is not supported for dtype %R, function(s) not found: cos, cosf", K_NAME, dtype);
            return 1;
        #endif
        }
        ...
        #ifdef KS_HAVE_cosl
            *(TYPE*)pR = cosl(vX);
        #elif defined(KS_HAVE_cos)
            *(TYPE*)pR = cos(vX);
        #elif defined(KS_HAVE_cosf)
            *(TYPE*)pR = cosf(vX);
        #else
            KS_THROW(kst_Error, "Kernel '%s' is not supported for dtype %R, function(s) not found: cosl, cos, cosf", K_NAME, dtype);
            return 1;
        #endif
        }
        ...
        for (i = 0; i < len; i++, pR += sR, pX += sX) {
            ...
            *(TYPE*)pR = cosf(vX);
        #else
            KS_THROW(kst_Error, "Kernel '%s' is not supported for dtype %R, function(s) not found: cosf128, cosl, cos, cosf", K_NAME, dtype);
            return 1;
        #endif
        }
